...
Raw paths from planning algorithms like grid-based or sampling-based planners often have turns.These paths also have curvature and are not feasible for robots with kinematic constraints.The sharp turns and discontinuous curvature cause difficulties for differential drive robots.These robots need to follow motion profiles and meet their nonholonomic constraints.Differential drive robots have trouble, with paths that have turns.Their motion profiles must be smooth and continuous.They also have to comply with their constraints.. To overcome these constraints, they use trajectory smoothing to turn discrete paths represented as waypoints into continuous and differentiable paths with the use of waypoints as prongs of a path representation ~\cite{ravankar2018path}. This process uses the interpolation methods to create smooth curves, which simultaneously meet the velocity conditions, deceleration conditions, and curvature conditions that are necessary to achieve a stable robotic movement. Removal of the sudden directional changes provided by the trajectory smoothing leads to a better tracking precision, less mechanical forces on the actuators, and increased efficiency of the navigation in general. 

Interpolation methods that use spline interpolations are really useful for making trajectories smooth. This is because spline interpolations are very flexible and make sense mathematically. There are a types of spline interpolations but the most common ones are cubic splines and B-splines.Cubic splines are good because they make curves that're smooth and have no sharp turns. They make sure that the position, velocity and acceleration of one part of the curve match the part. This means that cubic splines can move to the waypoints that we know and the transitions between different parts of the path are smooth.The cubic splines and B-splines are important, for interpolation methods based on spline interpolations. B-splines on the other hand offer a higher degree of flexibility, since they do not strictly interpolate the waypoints but approximate them. The nature of this allows B-splines to produce more smooth trajectories with finite curvature which is especially beneficial when motion tracking controllers are used in mobile robots. The use of spline-based smoothing makes the corresponding trajectory dynamically realistic and capable of execution by control algorithms, which in turn can be used by differential-drive robots when following planned paths safely and efficiently.
...
The A* method is really good at finding the path but it makes paths with a lot of sharp turns that are hard for robots to follow. These robots have wheels that move in ways so they cannot make these sharp turns easily. Because of this the path that A* finds is not good enough to use away. The A* path needs to be fixed and made better before we can use it on a robot or a robot, in a simulation.
...
Based on the comparison presented in Table~\ref{tab:path_planning_comparison}, the Improved A* algorithm was chosen as the global path planner for this project. Here are the reasons why:Dijkstras algorithm was ruled out because it explores every node, which's computationally expensive for large indoor maps. Standard A* was not enough since its output is jagged and grid-based requiring a lot of work.It also does not have safety margins. Pruning capabilities like Improved A* does.Other planners like RRT and RRT* were not picked because they are computationally costly behave unpredictably and produce paths.These planners are not suitable for 2D indoor environments, like ours.The Improved A* algorithm was selected as the global path planner. D* Lite, while well-suited for dynamic replanning, introduces unnecessary implementation complexity since dynamic obstacle avoidance in this project is managed at the local planner level using real-time sensor data. Potential Field methods were disqualified due to their well-known susceptibility to local minima traps in corridor-based layouts.
...
The hybrid strategy combines the speed of spline interpolation with the ability to make improvements when necessary. This approach always uses spline smoothing to make sure the result is physically possible whether or not the Particle Swarm Optimization refinement is used. The spline smoothing step is a part of this process and it always happens, so the hybrid strategy always gives a result that works, even if the Particle Swarm Optimization refinement is not used.
...
Recent research~\cite{cao2024pure} explores a way to control mobile robots. It uses a mix of Pure Pursuit steering and Soft Actor-Critic velocity control. This helps the robot adjust its speed based on the path it is taking.Some control methods that use Deep Reinforcement Learning can learn to behave in ways.. They need a lot of practice in a fake environment first. They are also hard to understand. Might not work well in new situations without more practice.On the hand Model Predictive Control is good, at handling limits and making robots behave predictably.. It needs smooth paths to work well.
\begin{table}[htbp]
	\centering
...
Model Predictive Control works with our system. It connects how smooth the planned path is to how the system follows it and controls its movements.The analysis we did shows that using paths is better than using direct waypoints.This is why we use paths in the controller.It helps in making the delivery of parcels smooth and stable when the system works on its own.The smoothed paths ensure that the system follows the path accurately.This accurate following leads to movements, during parcel delivery..

\subsection{Related Technology 5 -- Cellular Decomposition}

Cellular decomposition is common place planning approach, representation of the environment of classical environment representation in autonomous robot navigation~\cite{galceran2013survey}. The way divides the robot space into sections with no overlaps. These sections are called cells. This makes it easier to navigate because the open area is now organized.It is a method that makes computing more efficient. It also reduces crashes. Helps create paths in a systematic way.The cell structure helps robots move around without bumping into things. This way the robot can find its way more easily.Cells make navigation simpler. The robot area is split into cells. This helps with path creation.
...
Most new ways of doing things use a method called cellular decomposition, where each cell is a convex shape meaning it satisfies certain conditions that make it convex.~\cite{choset2001coverage}. Convex cells are really important because they make sure that we can draw a line between any two points inside the cell without running into anything that gets in the way. The rule, for convex cells can be stated like this:
\begin{equation}
\forall p_1, p_2 \in C_i, \quad \lambda p_1 + (1-\lambda) p_2 \in C_i, \quad \lambda \in [0,1]
\end{equation}

This property makes path planning much simpler. Improves how well we detect collisions.Cellular decomposition helps create a graph that shows how cells are connected. Each cell is a node. Cells that are next to each other have lines between them on the graph.To find the path we look for the one that costs the least to travel between cells that are connected.The cost is usually based on things like distance or obstacles.The cost function, in cell-based planning is typically defined as:
...
One of the things about cellular decomposition is that it can make complex places easier to navigate. This is very useful when we are using robots inside buildings. Cellular decomposition is also good at helping robots avoid obstacles. We can easily use it with methods, like Particle Swarm Optimization and Model Predictive Control.
...
ROS has become an used middleware for developing robotic applications. However it had some issues with scalability and real-time performance.This led to the creation of ROS2, which aims to meet the needs of autonomous robot development.Recent studies in robotics show that many ROS2-based systems focus on improving autonomy.These systems represent the state-of-the-art in robotics.We will be learning about them this semester.The latest reviews cover ROS2-based systems designed to enhance autonomy.They include ROS2-based systems.Autonomy is an area of focus, for these systems.ROS2 is being used to improve autonomy.~\cite{bonci2023ros2}. The papers emphasize modular designs which isolate perception, planning, and control so that a system can remain scalable and flexible without being caught by an inflexible design. In other works, the topic of practical designing and executing autonomous mobile robots in unstructured indoor settings is examined~\cite{gargouri2024cost}. These systems include hardware such as motors, sensors and controllers in addition to software modules in navigation and obstacle avoidance. Architectural and hardware-centric methods are both shown to be able to use ROS2 to support autonomous navigation. The comparison shows the usefulness of mixing ROS2 modular software stacks with practical hardware integration; this has been our first-hand experience, which we are applying to the design of our proposed autonomous parcel delivery system.
...
Several research studies have investigated various issues of autonomous mobile robot navigation which will help in the design and development of the proposed indoor parcel delivery robot system.

\begin{itemize}
\item \textbf{Hybrid Path Planning Strategies:} It has been shown in comparative studies that the combination of classical graph-based algorithms (e.g., A*) with optimization and learning-based approaches works well.~\cite{ajeil2020grid}.Research suggests that hybrid navigation leads to better path optimality, adaptability and computational efficiency.
\item \textbf{Path Smoothing and Trajectory Optimization:}  Work on trajectory smoothing techniques shows that raw paths typically consist of sharp turns and discontinuities~\cite{ravankar2018path}. The researches show that the spline based interpolation and curve fitting methods improve the smoothness of the motion, decrease the mechanical stresses and enhance the tracking accuracy.
\item \textbf{PSO-Based Path Optimization:} Particle Swarm Optimization (PSO) has been extensively studied for robot trajectories~\cite{song2020smooth}. Research shows that PSO can minimize path length, curvature variation, and energy consumption. Improved PSO variants demonstrate faster convergence and enhanced optimization performance.
