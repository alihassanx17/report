...
The A* algorithm generates redundant waypoints, which are reduced by using path pruning. Iterating through successive waypoints, the pruning algorithm eliminates intermediate spots that don't improve path quality. To ensure collision-free connectivity between non-consecutive waypoints, a line-of-sight check is carried out.


\subsubsection*{Backend Implementation File}

The A* algorithm is implemented in the backend as \texttt{astar\_modified.py}, which includes the modified A* path planning algorithm tailored for the particular restrictions of the project (dynamic safety margin, adaptive grid resolution, and 8-directional motion model, described in Section
~\ref{subsec:astar_parameters}).
...
The path planning performance criteria (path length, waypoint density, and computing efficiency) were used to assess the A* global planner in two simulated indoor test scenarios with varying levels of complexity and scale. For comparison, the pruned path (after removing redundant waypoints, see Section~\ref{subsec:astar_implementation}) and the raw A* output were both recorded.
...
\begin{itemize}[noitemsep]
\item \textbf{Identical paths between planners:} Because obstacles obstruct the straight start-to-goal line and require both planners to rely on the whole grid search, the Improved A* (IA*) planner and the conventional A* planner provide similar paths (same length and waypoint counts) in all circumstances. The computation time difference between the two planners is negligible.
\item \textbf{Effectiveness of pruning:} By eliminating pointless zigzag segments created by the 8-directional grid search, the path pruning step dramatically lowers waypoint density and shortens the overall path length:
...
Pruning will remove pointless waypoints, but the resulting path may have sharp curves and curvature discontinuities that don't fit a differential-drive robot's kinematic specifications. Particle Swarm Optimization (PSO) is used to further smooth the path in order to improve trajectory feasibility and lessen abrupt curves.

Each particle in the swarm represents a single candidate smoothed path, and the coordinates of the way points between the smoothed path particles are represented by the positional variables of each particle. In each iteration, the swarm adjusts the position of each particle according to the velocity equations, all while keeping an eye on a cost function that combines path length, smoothness and obstacle proximity. The swarm moves towards a path that is balancing these conflicting goals.
\subsection{Cost Function}
\label{subsec:pso_cost_function}

The PSO smoothing process optimizes a multi-objective cost function combining three weighted components:
...
Lower values of $J$ correspond to shorter, smoother, and safer paths. The obstacle penalty term $P$ increases sharply as a candidate path approaches or intersects an obstacle boundary, discouraging the swarm from converging on infeasible trajectories.
...
A Python implementation of Particle Swarm Optimization enhances the smoothness of navigational trajectories. Each particle represents a potential smoothed path, and the particle's search space is made up of the positional variables that make up the path's intermediate nodes. The swarm uses the velocity update equations to compute particle positions across a number of iterations.
...
The robot's motion is described using these basic equations:
...
While solving the above optimization problem, the controller must satisfy the following constraints:

\begin{itemize}[noitemsep]
	\item The predicted robot motion must satisfy the kinematic model (Equations~\ref{eq:mpc_kinematic_x}--\ref{eq:mpc_kinematic_theta}).
	\item \textbf{Velocity limits:} $0 \leq v \leq 0.22$~m/s and $-2.84 \leq \omega \leq 2.84$~rad/s.
	\item \textbf{Obstacle avoidance:} The predicted trajectory must maintain a safe distance from all known obstacles.
\end{itemize}
...
The impact of the MPC tuning profiles on the path-following performance of the robot was examined. Each profile modifies the robot's smoothness and degree of adherence to the intended course.
...
\begin{table}[H]
	...
	\end{tabular}
\end{table}
The \textbf{Calibrated Tuning} profile is appropriate for general navigation and offers balanced performance. The robot stays closer to the intended path while turning fewer corners thanks to the Glue-to-Path profile, which boosts the position-tracking weight. Out of the three profiles, the Strict Glue-to-Path profile produces the most precise path-following behavior by strengthening route tracking and reducing control variances.
...
To put it simply, the controller does this action during each control cycle:


\begin{enumerate}[noitemsep]
	\item Reads the robot's current position, heading, and speed, and reads the next section of the target path (from Phase 2).
	\item Predicts what will happen over the next $N$ steps using the kinematic model.
	\item Solves the optimization problem (Equation~\ref{eq:mpc_cost_p3}) using the selected profile's $Q$, $R$, and $S$ values to find the best sequence of moves.
...
\textbf{Routing based on roles.} The backend determines the user's role after a successful sign-in and routes them appropriately:

\begin{itemize}[noitemsep]
	\item \textbf{Users} are directed to the User Portal.
	\item \textbf{Administrators} are directed to the Admin Operations Hub.
\end{itemize}

\subsubsection{User Portal}
\label{sec:user_portal}
A typical user's main hub is the User Portal, which offers a live status card along with two essential actions: requesting a new delivery and examining the delivery history record. \textbf{Robot Status.} Without revealing any ROS2-level information to the user, the status card replicates the robot's current state: an idle robot is displayed as \textit{free}, while an active delivery is displayed as \textit{busy} along with its target room. The robot's current battery level is also shown as a percentage on the same card.
...
\textbf{2D Floor Plan Editor.} Allows administrators to edit and rename rooms in the building, and to update each room's coordinates so that a physical room maps to its exact pose on the map. Its detailed design is presented in Section~\ref{sec:floor_plan_editor}.

\textbf{Path Planner.} The path configuration screen. Built on an OpenStreetMap layout, it provides drawing tools for workspace boundaries and circular/polygonal obstacles (representing static walls or furniture), together with solver buttons that trigger A* search path calculations, cell decomposition, and Particle Swarm Optimization (PSO) path smoothing. Its full workflow is described in Section~3.4.3 (Phase 3: Web Platform Integration).

\textbf{Delivery Logs.} A historical audit list of all deliveries, showing order times, statuses (pending, transit, completed), and dispatch profiles.

\textbf{User Directory.} An admin tool to list all registered users, switch their authorization roles (e.g., upgrading a user to Administrator), or toggle their active status.
...
In order for the room choices supplied in the delivery request form to resolve to legitimate goal poses, the Floor Plan Editor is created to map each physical room to its corresponding coordinates in the robot's navigation frame. To keep room labels consistent with the actual facility, managers can rename rooms directly from this screen in addition to adjusting coordinates. Each room entry is purposefully divided into two separate groups of fields: (i) \emph{visual layout} fields (bounding-box width/height and on-map position, edited as percentage sliders) that control only the room's screen drawing, and (ii) \emph{ROS navigation} fields ($x$, $y$, $\theta$) that control the robot's actual goal pose. Table~\ref{tab:room_fields} illustrates this division, which enables the on-screen floor plan to be modified for visual clarity without risking accidental changes to the robot's calibrated delivery points.
\begin{table}[H]
	\centering
...
The backend server is implemented using the Python Flask framework and acts as the central coordination layer between the database, the path-planning algorithms, the web frontend, and the ROS2 robotic stack. Flask routes handle HTTP requests for map data, obstacle configuration, delivery management, robot telemetry, and user authentication and account management.
...
\begin{itemize}[noitemsep]
	\item Receives map, obstacle, and delivery request data in JSON format
	\item Executes the selected path-planning pipeline and returns computed trajectories
	\item Manages the robot's delivery Finite State Machine and dispatches navigation goals to ROS2
\end{itemize}

\subsubsection{Database Design}
\label{sec:db_design}

The system uses a relational SQLite database, accessed through the SQLAlchemy ORM. Table~\ref{tab:db_schema} summarizes the core tables.
...
The robot's delivery behaviour is governed by a Finite State Machine implemented in the backend, with its current state persisted in the robot state record described in Section~\ref{sec:db_design}. The FSM consists of four states: \textbf{free} (idle, parked at the home room), \textbf{en\_route\_to\_pickup} (moving to collect a parcel for room-to-room deliveries), \textbf{en\_route\_to\_dropoff} (moving to the destination room), and \textbf{returning} (navigating back to the home room after a completed delivery).

Transitions are triggered either by new delivery requests written to the database or by success/failure callbacks from the ROS2 Nav2 action client. Figure~\ref{fig:fsm} illustrates the complete state transition diagram, and Table~\ref{tab:fsm_transitions} summarizes the transition logic, with the pickup and drop-off transitions kept on separate rows to reflect the Room-to-Room and Home-to-Room delivery types described in Section~\ref{sec:user_portal}.
...
\end{longtable}
This preemption mechanism allows the robot to be redirected mid-return to a new delivery without first completing the trip home, improving overall delivery throughput. It also implements the queuing behaviour exposed to users on the User Portal (Section~\ref{sec:user_portal}): a request submitted while the robot is busy is simply stored as a pending order and picked up automatically by this same transition logic once the robot becomes free.


\subsubsection{Backend-to-ROS2 Communication}

The Flask backend communicates with the ROS2 stack asynchronously over WebSockets, avoiding the need to run ROS2 nodes inside the Flask process itself. A rosbridge\_websocket server (rosbridge\_suite) is run on port 9090, and the backend connects to it as a client using the roslibpy library. Figure~\ref{fig:ros_bridge} illustrates this communication path.
...
The setup of the virtual machine, operating system, ROS2 distribution, Gazebo simulator, and TurtleBot3 Waffle robot model are all covered in this section. No physical hardware was needed for this project because it is entirely simulation-based; the complete navigation stack was developed and tested in this virtual environment.


\subsection{Virtual Machine and Ubuntu Setup}
\label{subsec:vm_ubuntu_setup}

Because ROS2 and Gazebo are Linux natives, a Linux virtual machine was built on top of the host operating system using \textbf{Oracle VirtualBox}. The virtual machine was set up with a minimum of two processing cores, 4 GB of RAM, and network connectivity for package downloads. Since Ubuntu 22.04 LTS \textbf(Jammy Jellyfish) is the officially supported platform for the ROS2 Humble distribution, it was installed as the guest operating system. To guarantee compatibility with the software added in subsequent stages, the system packages were updated after installation.


\subsection{Gazebo 11 Classic Installation} \label{subsec:gazebo_install} \textbf{Gazebo Classic 11} was used as the physics simulation platform, as it is the version officially integrated with ROS2 Humble through the ROS-Gazebo bridge packages. The Gazebo simulator and its ROS2 integration packages were installed together, and the installation was verified by launching Gazebo directly to confirm it opened correctly. \subsection{TurtleBot3 Packages Installation} \label{subsec:turtlebot3_install} The \textbf{TurtleBot3 Waffle} robot model was used as the simulated robot platform. The complete set of TurtleBot3 packages was installed through the ROS2 package manager, and the robot model was configured to Waffle by setting the appropriate environment variable, which was also saved permanently so it persists across future terminal sessions. \subsection{ROS2 Workspace Creation} \label{subsec:ros2_workspace} All of the unique packages and nodes built for this project were housed in a special ROS2 workspace. This workspace had bespoke components, such as the MPC controller node, the trajectory publisher, and the custom world files. ROS2's build tool was then used to construct the workspace, and its setup file was sourced (and added to the shell startup file, after the ROS2 Humble source line), making all custom packages available in addition to the basic ROS2 installation in each new terminal session.
 \subsection{World Creation} \label{subsec:gazebo_world_creation} A custom Gazebo world was designed to simulate a realistic indoor environment for evaluating the proposed autonomous delivery system. The environment consists of four interconnected rooms linked by a central hallway, providing a combination of open spaces and confined passages to test the robot's navigation capabilities under different conditions. To resemble a real-world indoor facility, each room was furnished with a variety of Gazebo models, including office desks, chairs, sofas, cabinets, tables, waiting areas, and other indoor fixtures. One of the rooms was configured as a dedicated \textbf{robot room}, serving as the robot's starting location before executing delivery tasks. The diverse arrangement of furniture and room layouts creates a realistic environment containing both static obstacles and narrow navigation paths. To evaluate the robot's performance in dynamic environments, a walking person model was incorporated into the simulation to act as a moving obstacle. This dynamic obstacle introduces unpredictable motion, enabling the navigation system to be tested for real-time obstacle detection, avoidance, and path replanning. The complete simulation environment was launched together with the TurtleBot3 Waffle robot using a dedicated ROS~2 launch file, providing a realistic testbed for autonomous indoor navigation and delivery experiments.

\subsubsection*{Room Coordinate Mapping}

Each room in the simulated environment needed a known position so that it could later be selected as a pickup or drop-off point in the ParcelPath web interface (Phase 4). To determine these positions, a reference point was manually placed at the desired delivery location within each room in the Gazebo world. The corresponding $(x,y)$ coordinates of the selected point were then obtained directly from Gazebo's \textit{Property} panel, which displays the pose of the selected object in the world reference frame. This procedure was repeated for every room, resulting in a fixed set of coordinates that were subsequently registered in the 2D Floor Plan Editor as valid navigation goals.
...
The robot's home position, referred to as the \textbf{Robo Room}, was intentionally placed near the world origin at approximately $(0,0)$ to simplify initialization and navigation. Table~\ref{tab:room_coordinates} lists the recorded coordinates for all rooms in the simulated environment.
...
The link between the actual rooms in the Gazebo environment and the corresponding delivery locations in the ParcelPath web interface is established by these recorded coordinates. The stored coordinates are retrieved and converted into navigation goals when a user chooses a pickup or delivery room during operation, allowing the robot to navigate to the intended place on its own.
...
\section{Phase 7: MPPI Navigation}
...
where $\lambda$ is a temperature parameter controlling how strongly low-cost rollouts are favored over higher-cost ones -- a smaller $\lambda$ concentrates weight more heavily on the lowest-cost rollouts, while a larger $\lambda$ produces a more evenly averaged result.
...
\end{longtable}
These values were picked to balance speed and safety. Using 1000 sample paths gives the controller enough options to reliably judge which path is safest, without taking too long to compute at each step. The low temperature value ($\lambda = 1.0$) makes the robot commit strongly to the single best path it finds, instead of blending it with riskier options -- so it reacts quickly and decisively when the person gets close. Finally, the obstacle avoidance weight (50.0) is set much higher than the path-following weight (10.0), meaning the robot will happily move off the original path to avoid a collision rather than sticking rigidly to its route.
...
Once the dynamic obstacle has moved clear of the robot's path, the obstacle cost term no longer dominates, and the path tracking cost term draws the robot back onto the original global path. This layered design combines the strengths of both planners: the global planner ensures the robot always has an efficient, complete route to the goal, while the local MPPI planner ensures safe, reactive behavior in the presence of unpredictable, real-time disturbances  such as a person walking through the corridor that the global planner cannot foresee in advance.



\section{Phase 8: Rviz setting }

\subsection{RViz Visualization Configuration}

To visualize the outputs of the custom navigation stack during simulation, RViz2
was configured with dedicated Path displays for both the raw global planner
output and the optimized trajectory used for control. Figure~\ref{fig:rviz_settings}
shows the corresponding display settings.

The raw global path generated by the Improved A* planner is subscribed from the
\textit{/plan} topic and rendered using a green Lines style (RGB: 25, 255, 0).
The pruned and PSO-smoothed trajectory, subscribed from the \textit{/plan\_smoothed}
topic, is rendered separately using a blue Lines style (RGB: 0, 170, 255). Both
displays use a buffer length of 1, ensuring that only the most recently published
path is shown at any given time, rather than accumulating older paths from
previous planning cycles.

This configuration allows the raw A* output and the PSO-smoothed path to be
visually compared in real time within the same scene, making it possible to
directly observe the effect of the pruning and smoothing stages on path quality.
The Model Predictive Controller subscribes to the smoothed path on
\textit{/plan\_smoothed} and uses it as the reference trajectory for generating
the velocity commands that drive the robot's motion.
...
\begin{enumerate}
	...
	web\_video\_server. This stream is consumed directly by the Live Telemetry
	module of the Admin Operations Hub.
	
	\item \textbf{Backend and Frontend Servers:} Finally, the Flask backend
	server (server.py) and the Flutter frontend (ParcelPath) are
	started, completing the software stack and exposing the web application to
	the user.
\end{enumerate}
...
live telemetry and camera feed. A Finite State Machine managed delivery
dispatch, queuing, and mid-return preemption, communicating with ROS2 throughrosbridge.The complete system was tested in a custom Gazebo world with TurtleBot3,demonstrating reliable path planning, smooth trajectory tracking, and
effective obstacle avoidance, all fully simulated without physical hardware.